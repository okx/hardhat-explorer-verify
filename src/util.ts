...
import { CustomChain } from './types/type';
import { chainConfig } from './config/ChainConfig';

export function buildContractUrl(browserURL: string, contractAddress: string): string {
...
export async function printSupportedNetworks(customChains: CustomChain[]) {
    ...
    // custom networks
    const customNetworks = customChains.map(({ network, chainId }) => [network, chainId]);

    const customNetworksTable =
        customNetworks.length > 0
            ? table([[chalk.bold('network'), chalk.bold('chain id')], ...customNetworks])
