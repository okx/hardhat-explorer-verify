...
import { TASK_VERIFY_VERIFY } from '../task/constants';

import { callEtherscanApi, getEtherscanInstance, RESPONSE_OK, verifyAndGetStatus } from './etherscan-api';
 ...
 * @returns
 */
export async function verify(args: any, hre: HardhatRuntimeEnvironment) {
    ...
    if (await isTransparentOrUUPSProxy(provider, proxyAddress)) {
        ...
        console.log('Beacon proxy verify.')
    ...
    async function hardhatVerify(address: string) {
        return await hre.run(TASK_VERIFY_VERIFY, { ...args, address });
    }
}
...
async function fullVerifyTransparentOrUUPS(
    ...
    async function verifyAdmin() {
        ...
        if (!isEmptySlot(adminAddress)) {
            console.log(
                chalk.bgGreenBright(`Verifying proxy admin: ${adminAddress}`),
            );
    ...
    async function verifyTransparentOrUUPS() {
        console.log(
            chalk.bgGreenBright(`Verifying transparent or UUPS proxy: ${proxyAddress}`),
        );
...
async function fullVerifyBeaconProxy(
    ...
    async function verifyBeaconProxy() {
        console.log(
            chalk.bgGreenBright(`Verifying beacon proxy: ${proxyAddress}`),
        );
        await verifyWithArtifactOrFallback(
            hre,
            hardhatVerify,
...
async function verifyContractWithConstructorArgs(
    ...
    try {
        ...
        if (status.isSuccess()) {
            console.log(
                chalk.bgGreenBright(`Successfully verified contract ${artifact.contractName} at ${address} with constructor args.`),
            );
            console.log(
                chalk.redBright(`Check Link:`),
                chalk.yellowBright(`${etherscan.browserUrl}/address/${address}`)
            )
        } else {
        ...
        if (e.message.toLowerCase().includes('already verified')) {
            console.log(
                chalk.bgYellow(
                    `Contract at ${address} already verified.`
                )
            );
        } else {
