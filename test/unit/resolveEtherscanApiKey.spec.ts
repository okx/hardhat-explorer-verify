...
describe('Etherscan API Key resolution', () => {
    ...
    describe('provide multiple api keys', () => {
        it('can retrieve different keys depending on --network', () => {
            ...
            assert.equal(resolveEtherscanApiKey(apiKey, 'mainnet'), 'mainnet-testtoken');
            assert.equal(resolveEtherscanApiKey(apiKey, 'goerli'), 'goerli-testtoken');
        });
    });
    ...
    describe('provide no api key', () => {
        ...
        it('should throw if api key root is empty string', () => {
            assert.throws(() => resolveEtherscanApiKey('', 'goerli'), expectedBadApiKeyMessage);
        });
